...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!res.ok) {
      ...
      console.error('Didit API error:', res.status, errorText)
      return new Response(
    ...
    console.error('Didit lookup failed:', error)
    return new Response(
      JSON.stringify({ error: 'Didit lookup failed', details: error.message }),
