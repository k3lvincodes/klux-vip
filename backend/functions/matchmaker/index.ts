...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Get passenger phone for SMS notification
    ...
    // Notify customer that driver is assigned via SMS
    if (passengerProfile?.phone_number) {
      supabase.functions.invoke('sms-notifications', {
        body: {
          type: 'driver_assigned',
          to: passengerProfile.phone_number,
          data: {
            customer_name: passengerProfile.first_name || 'Customer',
            driver_name: `${selectedDriver.first_name} ${selectedDriver.last_name}`.trim(),
            pickup_address: ride.pickup_address,
            eta: '5-10 min',
          },
        },
      }).then().catch((err) => console.error('Failed to send driver-assigned SMS:', err))
    }

    // Notify customer that driver is assigned via email
    const { data: passengerEmail } = await supabase
      .from('profiles')
      .select('email')
      .eq('id', ride.passenger_id)
      .single()

    if (passengerEmail?.email) {
      ...
      }).then().catch((err) => console.error('Failed to send driver-assigned email:', err))
    }
    ...
    }).then().catch((err) => console.error('Failed to insert driver notification:', err))
